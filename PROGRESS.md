...
## 2026-04-29 12:08 进度：提示词助手动态化

- 已将提示词助手重构为动态类型骨架，不再只服务摄影 / 人像 / 产品。
- 当前流程：内容类型 → 结构版式 → 风格质感 → 文字控制。
- 攻略图、信息图、小红书封面、PPT配图、教程步骤图等类型已有独立词库。
- 不增加标题文字输入框，文字相关能力通过「文字方式」选项表达。
- 设计类类型默认隐藏摄影专属选项；摄影类类型保留镜头、光线、材质质感。
- 当前版本：`image-studio-pwa-v29`。

## 2026-04-29 12:50 进度：提示词助手连贯性优化

- 已将提示词助手预览从词条堆叠改为自然中文描述。
- 新增本地提示词整理器：按内容类型套用不同表达骨架，保证主题、结构、版式、风格、文字方式和细节控制按合理顺序输出。
- 新增基础文字冲突处理：选择「不要文字」后会覆盖其他文字方式；再选择其他文字方式会移除「不要文字」。
- 为电商详情图、公众号首图、清单卡片、路线地图、插画图标补齐专属词库和模板类型，减少 fallback 复用。
- 当前版本：`image-studio-pwa-v33`。
